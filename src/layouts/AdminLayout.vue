      ...
      <router-view v-slot="mainprops">
        <div class="viewwrapper">
          <div class="wrap-for-popup">
            ...
            >
              <sl-tab v-if="state.validHandlers.length > 0" slot="nav" @click="navigateSubRoute()">
                <sl-icon name="database-fill"></sl-icon>
              </sl-tab>
              <template v-for="handler in state.validHandlers" :key="handler['name']">
                <sl-tab slot="nav" @click="navigateSubRoute(handler['route'])">
                  <sl-icon :title="handler['name']" :name="handler['icon']"></sl-icon>
                </sl-tab>
              </template>
              ...
              >
                <view-wrapper :component="mainprops.Component"></view-wrapper>
              </sl-tab-panel>
            </sl-tab-group>
          </div>
        </div>
