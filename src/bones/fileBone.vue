<template>
  ...
  >
    <div v-if="state.droparea" class="droparea">
      {{ $t("bone.dragFile") }}
    </div>
    <sl-button
    ...
    <sl-button v-if="value?.['dest']" :title="$t('bone.download')" @click="downloadFile">
      <sl-icon slot="prefix" name="download"></sl-icon>
    </sl-button>
    <div v-if="!boneState.isEmpty && value?.['dest']" class="box">
      <div v-if="value?.['dest']?.['mimetype'].includes('image')" class="preview">
        <vi-image :src="Request.downloadUrlFor(value)" popup></vi-image>
      </div>
