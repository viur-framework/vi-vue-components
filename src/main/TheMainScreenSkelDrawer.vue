<template>
  ...
  >
    ...
    <template v-else>
      <div v-if="dbStore.state['skeldrawer.entry']['mimetype']" class="image-wrap">
        <img
          v-if="
          ...
          :src="getImageUrl()"
        />
        <sl-icon v-else name="file-richtext-fill"></sl-icon>
      ...
      <div v-for="(bone, boneName) in dbStore.state['skeldrawer.entry']" class="details">
        <span class="details-headline">{{ dbStore.state["skeldrawer.structure"][boneName]?.["descr"] }}:</span>
